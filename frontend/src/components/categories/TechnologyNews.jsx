...
// Component to display only technology-related news
export default function TechnologyNews() {
    // State to hold filtered technology news
    const [news, setNews] = useState([]);

    // Filter and load technology news when component mounts
    useEffect(() => {
        setNews(sampleNews.filter((item) => item.category === "Technology"));
    }, []);
        ...
        <main className="relative z-10 pt-16 transition-all lg:ml-64">
            <div className="px-4 sm:px-6 lg:px-10 py-8">
                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {news.length > 0 ? (
                        // Show each technology news item
                        news.map((item) => (
                            <div
                                key={item.id}
                        ...
                        // Message if no technology news found
                        <p className="text-gray-500">No Technology news available</p>
                    )}
                </div>
            </div>
        </main>
