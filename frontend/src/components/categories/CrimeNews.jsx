...
// Component to display only crime-related news
export default function CrimeNews() {
    // State to hold filtered crime news
    const [news, setNews] = useState([]);

    // Filter and load crime news when component mounts
    useEffect(() => {
        setNews(sampleNews.filter((item) => item.category === "Crime"));
    }, []);
            ...
            <div className="px-4 sm:px-6 lg:px-10 py-8">
                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {news.length > 0 ? (
                        ...
                        // Message if no crime news found
                        <p className="text-gray-500">No Crime news available</p>
                    )}
                </div>
            </div>
        </main>
