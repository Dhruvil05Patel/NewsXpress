...
// Component to display only politics-related news
export default function PoliticsNews() {
    // State to hold filtered politics news
    const [news, setNews] = useState([]);

    // Filter and load politics news when component mounts
    useEffect(() => {
        setNews(sampleNews.filter((item) => item.category === "Politics"));
    }, []);
            ...
            <div className="px-4 sm:px-6 lg:px-10 py-8">
                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {news.length > 0 ? (
                        ...
                        // Message if no politics news found
                        <p className="text-gray-500">No Politics news available</p>
                    )}
                </div>
            </div>
        </main>
