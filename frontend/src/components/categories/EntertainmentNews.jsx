...
// Component to display only entertainment-related news
export default function EntertainmentNews() {
    // State to hold filtered entertainment news
    const [news, setNews] = useState([]);

    // Filter and load entertainment news when component mounts
    useEffect(() => {
        setNews(sampleNews.filter((item) => item.category === "Entertainment"));
    }, []);
        ...
        <main className="relative z-10 pt-16 transition-all lg:ml-64">
            <div className="px-4 sm:px-6 lg:px-10 py-8">
                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {news.length > 0 ? (
                        // Show each entertainment news item
                        news.map((item) => (
                            <div
                                key={item.id}
                        ...
                        // Message if no entertainment news found
